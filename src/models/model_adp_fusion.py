...
class AdpFusionModel(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.encoder.load_adapter(self.output_dir+"/Adp_T", with_head=False)
        prot_adp_string = []
        for adp_name_key in self.num_labels_protected:
            prot_adp_string.append("prot"+str(adp_name_key))
            self.encoder.load_adapter(self.output_dir+"/prot"+str(adp_name_key), with_head=False)
        if len(prot_adp_string)==1:
            adapter_setup = Fuse("task", prot_adp_string[0])
        else:
            adapter_setup = Fuse("task", prot_adp_string[0],prot_adp_string[1])
        self.encoder.add_adapter_fusion(adapter_setup)
        self.encoder.set_active_adapters(adapter_setup)
        self.encoder.train_adapter_fusion(adapter_setup)
